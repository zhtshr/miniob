...
RC PaxRecordPageHandler::get_record(const RID &rid, Record &record)
{
  // your code here
  Bitmap bitmap(bitmap_, page_header_->record_capacity);
  int record_col_total_size = page_header_->record_real_size+4;
  if(bitmap.get_bit(rid.slot_num)) {
    // @assume
...
RC PaxRecordPageHandler::get_chunk(Chunk &chunk)
{
  ...
  for(int i=0; i<chunk.column_num(); i++) {
    ...
    int a;
    while(true){
      a = bitmap.next_setted_bit(j);
      j = a+1;
      // @assume
      if(a == -1 || chunk.column_ptr(i)->count() >= chunk.capacity() ){
        break;
      }
      chunk.column_ptr(i)->append_one(get_field_data(a,col_id));
    }
